<!--



📚 Aula Explicada Passo a Passo: Navbar + Menu Hambúrguer em React + TailwindCSS
🎯 O que vamos aprender
1. Como criar uma barra de navegação (navbar) em React.
2. O que é o menu hambúrguer e como ele funciona em telas pequenas.
3. Como usar o estado (useState) para abrir e fechar a sidebar.
4. Como o TailwindCSS nos ajuda com classes prontas para layout e responsividade.



1️⃣ Estrutura da Navbar
Pensa assim:
. A navbar é um retângulo fixo no topo da página, com:
  . O logo ou nome à esquerda.
  . Os links no meio ou à direita.
  . O botão hambúrguer que aparece só no celular.

👉 Em React, cada pedaço da tela é um componente. Então criamos um componente chamado Navbar.

import React, { useState } from "react"

export default function Navbar() {
  const [open, setOpen] = useState(false) // estado para controlar se o menu está aberto ou fechado

  return (
    <nav className="fixed w-full flex items-center justify-between px-6 py-4 bg-black/80 text-white z-40">
      <h1 className="text-2xl font-bold">
        <span className="text-sky-300">Mikael</span> Rocha
      </h1>
    </nav>
  )
}

 
🔎 Aqui temos:
. useState(false) → começa fechado (false).
. nav → a barra de navegação.
. fixed w-full → fixa no topo e ocupa a largura da tela inteira.
. px-6 py-4 → espaçamento interno.
. bg-black/80 → fundo preto com transparência.

 


2️⃣ Links da navbar:

Agora colocamos os links de navegação.
Queremos que eles só apareçam em telas maiores (computador).
No Tailwind, fazemos isso com hidden md:flex.

<ul className="hidden md:flex gap-8 text-lg">
  <li><a href="#sobre">Sobre</a></li>
  <li><a href="#projeto">Projetos</a></li>
  <li><a href="#skills">Skills</a></li>
  <li><a href="#contato">Contato</a></li>
</ul>


🔎 Aqui:
. hidden → por padrão fica invisível.
. md:flex → a partir do tamanho md (tablet para cima) vira flex e aparece.
. gap-8 → espaçamento entre os itens.

3️⃣ Botão Hambúrguer

Esse é o botão que aparece somente no celular.
Ele abre o menu lateral quando clicamos.

<button
  className="md:hidden text-3xl"
  onClick={() => setOpen(true)}
>
  <i className="bx bx-menu"></i>
</button>


🔎 Aqui:
. md:hidden → o botão só aparece em telas menores que md.
. text-3xl → aumenta o tamanho do ícone.
. onClick={() => setOpen(true)} → quando clicamos, mudamos o estado open para true.
. i className="bx bx-menu" → ícone do hambúrguer (as 3 listrinhas).




4️⃣ Sidebar (menu lateral no mobile):

Agora o truque principal: só queremos mostrar a sidebar se open === true.
No React, usamos {open && (...)}.

{open && (
  <div className="fixed top-0 left-0 w-2/3 h-full bg-black/90 text-white flex flex-col p-6 z-50">
    {/* Botão fechar */}
    <button
      className="self-end text-3xl mb-6"
      onClick={() => setOpen(false)}
    >
      <i className="bx bx-x"></i>
    </button>

    {/* Links mobile */}
    <ul className="flex flex-col gap-6 text-lg">
      <li><a href="#sobre" onClick={() => setOpen(false)}>Sobre</a></li>
      <li><a href="#projeto" onClick={() => setOpen(false)}>Projetos</a></li>
      <li><a href="#skills" onClick={() => setOpen(false)}>Skills</a></li>
      <li><a href="#contato" onClick={() => setOpen(false)}>Contato</a></li>
    </ul>
  </div>
)}


🔎 Detalhes importantes:
. {open && (...)} → significa “se open for true, renderiza isso”.
. fixed top-0 left-0 → fixa a sidebar no canto superior esquerdo.
. w-2/3 h-full → ocupa 2/3 da tela em largura e 100% da altura.
. bg-black/90 → fundo preto semi-transparente.
. z-50 → garante que aparece por cima de tudo.
. setOpen(false) → fecha o menu quando clicamos no X ou em um link.



5️⃣ Como tudo funciona junto (fluxo):
1. Página começa → open = false → sidebar não aparece.
2. Clicamos no botão hambúrguer → setOpen(true) → agora open = true.
3. O React re-renderiza → a condição {open && (...)} passa a ser verdadeira → sidebar aparece.
4. Dentro da sidebar, clicamos no X ou em algum link → setOpen(false).
5. React re-renderiza → open = false → sidebar some. 🎉



6️⃣ Tailwind na prática:

. Responsividade:
  . hidden md:flex → mostra só no desktop.
  . md:hidden → mostra só no celular.

. Layout da sidebar:
  . flex flex-col → organiza os links em coluna.
  . gap-6 → espaço entre os itens.

. Estilização do fundo:
  . bg-black/90 → cor preta transparente.
  . fixed top-0 left-0 → cobre a tela inteira (como um overlay).

✅ Resumo Final
. Criamos um estado (open) para controlar se o menu lateral está aberto ou fechado.
. No desktop → mostramos os links direto.
. No mobile → mostramos um botão hambúrguer que abre a sidebar.
. O React usa a lógica {open && (...)} para decidir se renderiza ou não a sidebar.
. O Tailwind nos ajudou com responsividade, espaçamentos e fundo fixo.

















🧠 Aula Completa: Validação de Formulário no React — Explicada Passo a Passo:

toda essa explicação é baseada na solução usada nesta aula.

🎯 Objetivo da aula:
Aprender a criar um formulário funcional com validação personalizada 
no React, sem precisar de bibliotecas externas, entendendo a lógica por trás de cada passo.

Essa aula foi feita para compreender o raciocínio da validação, e não apenas copiar código.


🧩 1. O que é validação de formulário:
A validação de formulário serve para verificar se os dados 
digitados pelo usuário estão corretos e completos antes de enviar o formulário.

Exemplo:
. Nome não pode ficar vazio;
. E-mail precisa conter “@” e “.”;
. A mensagem não pode estar em branco.

Sem validação, o usuário pode enviar o formulário com campos vazios ou errados, 
e isso pode gerar problemas depois (erros, mensagens sem contato, etc).



⚙️ 2. Criando os estados:

No React, usamos useState para guardar informações que mudam.
No caso de formulários, temos dois tipos de dados pra guardar:

. Os valores dos campos (nome, e-mail, mensagem);
. As mensagens de erro que aparecem caso algo esteja errado.

const [nome, setNome] = useState("");
const [email, setEmail] = useState("");
const [mensagem, setMensagem] = useState("");

// Estados para armazenar os erros
const [erroNome, setErroNome] = useState("");
const [erroEmail, setErroEmail] = useState("");
const [erroMensagem, setErroMensagem] = useState("");


📘 Por que usar estados?
Porque o React precisa “reagir” quando algo muda.
Ou seja, quando o usuário digita, o estado atualiza o valor automaticamente, e quando há um erro, o estado exibe a mensagem na tela.



🧠 3. Entendendo o funcionamento da função de validação:

Quando o usuário clicar no botão Enviar, uma função será executada para checar se os campos estão corretos.
Essa função será chamada, por exemplo, validarFormulario.


🔸 Variável de controle (temErro):
No início da função, criamos uma variável que vai controlar se há algum erro no formulário:

let temErro = false;


Se nenhum erro for encontrado, essa variável continua sendo false, e o formulário é enviado.
Mas se qualquer campo tiver erro, ela muda para true, e o envio é interrompido.

💡 Por que usar essa variável em vez de return direto?
Porque o return para a função imediatamente, e só permitiria validar um campo por vez.
Usando a variável temErro, conseguimos verificar todos os campos e marcar os erros 
de uma vez só, mesmo que existam vários.

No final, usamos uma condição pra impedir o envio caso algum erro tenha ocorrido:
js:
if (temErro) return; // se tiver erro, a função encerra aqui



🧽 4. Limpando mensagens antigas:

Antes de começar a validar, é importante limpar os erros anteriores, 
senão as mensagens antigas podem continuar aparecendo mesmo que o usuário já 
tenha corrigido os campos.
js:
setErroNome("");
setErroEmail("");
setErroMensagem("");


📘 Explicação mais profunda:
Mesmo que o usuário digite tudo certo na segunda tentativa, os estados de erro 
continuam guardando as mensagens da tentativa anterior.
Por isso, é fundamental limpá-los no início da função — pra garantir que a validação comece sempre “do zero”.



✅ 5. Validando os campos um por um:

Agora vamos criar as verificações (as condições if) para cada campo.

🧍‍♂️ Campo “Nome”
if (nome.trim() === "") {
  setErroNome("Por favor, digite seu nome.");
  temErro = true;
}


📘 Explicação detalhada:
. nome.trim() remove espaços antes e depois do texto (por exemplo, se o usuário digitar só “ ”);
. === "" verifica se o campo está realmente vazio;
. Se estiver, mostramos a mensagem de erro e marcamos que houve erro com temErro = true.

Traduzindo:
“Se o nome estiver em branco, mostre a mensagem e marque que há erro.”


📧 Campo “Email”:
if (!email.includes("@") || !email.includes(".")) {
  setErroEmail("Digite um email válido.");
  temErro = true;
}


🔍 Entendendo o includes():
O método .includes() verifica se um texto contém um certo caractere ou palavra.

Ele retorna:
. true se encontrar;
. false se não encontrar.

Exemplo rápido:
"banana".includes("b") // true
"banana".includes("z") // false


No nosso caso:
. email.includes("@") → verifica se o e-mail contém o símbolo “@”;
. email.includes(".") → verifica se contém um ponto “.” (parte final do domínio).

⚙️ O papel do ! (negação):
O ! inverte o valor.

Ou seja:
. Se email.includes("@") for true, o ! transforma em false;
. Se for false, o ! transforma em true.

Por isso, !email.includes("@") significa “se NÃO existir @”.

⚙️ O papel do || (ou):

O operador || significa “ou” — ele só precisa de uma condição verdadeira pra entrar no if.
Mas aqui estamos usando condições negativas, então:

Se não tiver “@” OU não tiver “.”, já é considerado inválido.
       Isso garante que o email precisa ter os dois — o “@” e o “.” — para passar na validação.



💬 Campo “Mensagem”:
if (mensagem.trim() === "") {
  setErroMensagem("Digite uma mensagem antes de enviar.");
  temErro = true;
}


📘 Explicação completa:

Usamos .trim() para eliminar espaços e garantir que o campo realmente tenha texto;
. A condição === "" verifica se o campo está vazio;
. Se estiver, exibimos a mensagem de erro;
. E marcamos o erro com temErro = true pra impedir o envio.



⛔ 6. Bloqueando o envio se houver erros:

Depois de validar todos os campos, verificamos se a variável de controle temErro virou true.

if (temErro) {
  return; // interrompe a função e impede o envio
}


🧠 Por que isso é importante?
Porque com essa verificação, conseguimos validar todos os campos de 
uma só vez, mostrar todas as mensagens de erro, e só enviar o formulário se estiver tudo certo.


📤 7. Enviando o formulário manualmente:
Por padrão, os formulários HTML enviam os dados automaticamente, 
mas no React, usamos eventos controlados.

Por isso, usamos:
js:
evento.preventDefault();


👉 Isso impede o envio automático.

E, no final, se tudo estiver certo, enviamos manualmente com:
js:
evento.target.submit();


📘 Entendendo linha por linha:
. evento → é o objeto que contém todas as informações da ação (neste caso, 
o clique no botão ou o envio do formulário);
. evento.target → representa o próprio formulário que foi enviado;
. .submit() → envia o formulário de forma manual, como se fosse o envio 
automático do HTML.

Usamos essa forma manual pra garantir que só envie se a validação estiver 100% certa.



💻 8. Exemplo completo:
import React, { useState } from "react";

export default function Formulario() {
  const [nome, setNome] = useState("");
  const [email, setEmail] = useState("");
  const [mensagem, setMensagem] = useState("");

  const [erroNome, setErroNome] = useState("");
  const [erroEmail, setErroEmail] = useState("");
  const [erroMensagem, setErroMensagem] = useState("");

  function validarFormulario(evento) {
    evento.preventDefault();

    // Limpando mensagens antigas
    setErroNome("");
    setErroEmail("");
    setErroMensagem("");

    let temErro = false; // variável de controle

    // Validação do nome
    if (nome.trim() === "") {
      setErroNome("Por favor, digite seu nome.");
      temErro = true;
    }

    // Validação do email
    if (!email.includes("@") || !email.includes(".")) {
      setErroEmail("Digite um email válido.");
      temErro = true;
    }

    // Validação da mensagem
    if (mensagem.trim() === "") {
      setErroMensagem("Digite uma mensagem antes de enviar.");
      temErro = true;
    }

    // Se tiver algum erro, para aqui
    if (temErro) return;

    // Se tudo estiver certo, envia o formulário
    evento.target.submit();
  }

  return (
    <form onSubmit={validarFormulario}>
      <input
        type="text"
        placeholder="Nome"
        value={nome}
        onChange={(e) => setNome(e.target.value)}
      />
      {erroNome && <p>{erroNome}</p>}

      <input
        type="email"
        placeholder="Email"
        value={email}
        onChange={(e) => setEmail(e.target.value)}
      />
      {erroEmail && <p>{erroEmail}</p>}

      <textarea
        placeholder="Mensagem"
        value={mensagem}
        onChange={(e) => setMensagem(e.target.value)}
      />
      {erroMensagem && <p>{erroMensagem}</p>}

      <button type="submit">Enviar</button>
    </form>
  );
}

🧾 9. O que você aprendeu

✅ Como usar estados para campos e mensagens de erro
✅ O que é a variável de controle e por que ela é essencial
✅ Por que limpar os erros no início da função
✅ Como funcionam includes(), ! e ||
✅ A diferença entre usar return e uma variável de controle
✅ Como impedir o envio automático e enviar manualmente com evento.target.submit()

💬 Conclusão

Agora você domina a base completa da validação manual no React — 
entendendo a lógica por trás, não só o código.
Na próxima vez que precisar validar qualquer formulário, é só olhar 
essas anotações e adaptar as condições conforme o caso.

-->








<!--
Anotação de 23/01/2026
-->

<!--
========================================
📄 IMPLEMENTAÇÃO DE CURRÍCULO PARA DOWNLOAD
========================================

🧠 OBJETIVO:
Permitir que o visitante do portfólio baixe meu currículo em PDF
de forma simples, direta e profissional.

📁 ESTRUTURA CORRETA:
O arquivo do currículo deve ficar dentro da pasta "public",
pois tudo que está em "public" é servido diretamente pelo navegador.

Exemplo:
public/
 └── curriculo/
     └── Currículo-Mikael-Julião-Rocha.pdf

⚠️ IMPORTANTE:
- O PDF precisa estar válido (com conteúdo).
- Se o PDF estiver vazio ou corrompido, o download funciona,
  mas o arquivo não abre corretamente.
- Sempre testar abrindo o PDF direto pelo navegador.

🔗 COMO FUNCIONA O LINK:
Usamos a tag <a> com o atributo "download".
Isso faz o navegador baixar o arquivo em vez de apenas abrir.

Exemplo em React:
<a
  href="/curriculo/Currículo-Mikael-Julião-Rocha.pdf"
  download
>
  Baixar currículo
</a>

📌 OBSERVAÇÕES IMPORTANTES:
- O caminho começa com "/" porque aponta para a pasta public.
- O nome do arquivo precisa ser exatamente igual (maiúsculas,
  minúsculas e acentos contam).
- Evitar renomear PDFs manualmente dentro do VS Code se o arquivo
  estiver corrompido; prefira gerar novamente o PDF.

🧪 TESTES REALIZADOS:
✔ Arquivo abre direto no navegador
✔ Download funciona
✔ PDF abre com conteúdo correto

🚀 APRENDIZADO:
Arquivos estáticos (PDF, imagens, vídeos) devem ficar em "public"
para acesso direto via URL em projetos React com Vite.
-->



























<!--
=========================================================
📱 ENVIO DE FORMULÁRIO PARA WHATSAPP (ESTUDO COMPLETO)
=========================================================

🧠 OBJETIVO:
Permitir que dados digitados pelo usuário em um formulário
(nome, email, mensagem, etc.) sejam enviados diretamente para
uma conversa no WhatsApp, sem backend e sem API externa.

Essa abordagem é ideal para:
- Portfólios
- Landing pages
- Contato profissional
- Projetos front-end simples e eficientes

---------------------------------------------------------
1️⃣ CONCEITO PRINCIPAL — CLICK TO CHAT
---------------------------------------------------------

O WhatsApp oferece uma funcionalidade oficial chamada
"Click to Chat".

Ela permite iniciar uma conversa com qualquer número
utilizando apenas uma URL, sem necessidade de login,
API, token ou backend.

Estrutura base da URL:
https://wa.me/NUMERO?text=MENSAGEM

✔ NUMERO → telefone com DDI + DDD (sem espaços ou símbolos)
✔ MENSAGEM → texto que aparecerá já digitado na conversa

Exemplo simples:
https://wa.me/5585986066467?text=Olá%20tudo%20bem

Ao acessar essa URL:
- O WhatsApp Web (desktop) ou App (mobile) é aberto
- A conversa é iniciada automaticamente
- A mensagem já aparece preenchida

---------------------------------------------------------
2️⃣ POR QUE NÃO USAR API DO WHATSAPP?
---------------------------------------------------------

❌ A API oficial do WhatsApp exige:
- Conta WhatsApp Business
- Configuração técnica avançada
- Backend obrigatório
- Custos financeiros
- Processo de aprovação

✅ Para portfólio e contato profissional:
- Click to Chat é simples
- Funciona em web e mobile
- Não exige backend
- Não gera custo
- Resolve o problema com menos complexidade

👉 Regra importante de engenharia:
"Se uma solução simples resolve, não complique."

---------------------------------------------------------
3️⃣ CONTEXTO REAL DO PROJETO
---------------------------------------------------------

Neste projeto, o formulário NÃO envia dados para um servidor.

Em vez disso:
- Os dados digitados pelo usuário são usados apenas
  para montar uma mensagem
- O navegador redireciona para o WhatsApp
- O WhatsApp assume a conversa

Nenhuma informação é armazenada ou processada no backend.

---------------------------------------------------------
4️⃣ BLOCO PRINCIPAL DE CÓDIGO (CASO REAL)
---------------------------------------------------------

const texto = `
Olá! Me chamo ${nome}.
Meu email é ${email}.

Mensagem:
${mensagem}
`;

const telefone = "5585986066467";
const url = `https://wa.me/${telefone}?text=${encodeURIComponent(texto)}`;

window.open(url, "_blank");

---------------------------------------------------------
5️⃣ EXPLICANDO A MENSAGEM (TEMPLATE STRING)
---------------------------------------------------------

🔹 const texto = `...`

Foi utilizada uma TEMPLATE STRING (crase `) para:
- Inserir variáveis dinamicamente (${nome}, ${email}, ${mensagem})
- Manter quebras de linha
- Criar uma mensagem organizada e legível

O texto final que chega ao WhatsApp é exatamente
o que o usuário digitou, apenas estruturado melhor.

Exemplo final no WhatsApp:

Olá! Me chamo João.
Meu email é joao@email.com.

Mensagem:
Gostei do seu portfólio e quero conversar.

---------------------------------------------------------
6️⃣ FORMATO DO NÚMERO DE TELEFONE
---------------------------------------------------------

🔹 const telefone = "5585986066467";

Formato obrigatório:
DDI + DDD + número (sem espaços, traços ou parênteses)

55 → Brasil  
85 → Ceará  
986066467 → número do WhatsApp  

Se o formato estiver incorreto:
❌ a conversa não abre
❌ o redirecionamento falha

---------------------------------------------------------
7️⃣ O QUE É encodeURIComponent (PASSO MAIS IMPORTANTE)
---------------------------------------------------------

🔹 encodeURIComponent(texto)

É uma função nativa do JavaScript que converte textos
em um formato seguro para ser usado dentro de URLs.

Ela resolve problemas com:
- Espaços
- Quebras de linha
- Acentos (á, é, í, ç, ã)
- Caracteres especiais (?, &, %, #)

Sem encodeURIComponent:
❌ a URL quebra
❌ parte da mensagem some
❌ o WhatsApp pode não abrir corretamente

Exemplo prático:

Texto original:
"Olá, tudo bem?"

Sem encode:
https://wa.me/...text=Olá, tudo bem?
❌ URL inválida

Com encodeURIComponent:
https://wa.me/...text=Ol%C3%A1%2C%20tudo%20bem%3F
✔ URL válida

👉 Conclusão:
encodeURIComponent garante que o WhatsApp receba
exatamente a mensagem digitada pelo usuário.

---------------------------------------------------------
8️⃣ CONSTRUÇÃO FINAL DA URL
---------------------------------------------------------

🔹 const url = `https://wa.me/${telefone}?text=${encodeURIComponent(texto)}`;

Aqui unimos:
- URL oficial do WhatsApp
- Número do destinatário
- Mensagem codificada corretamente

Essa URL por si só já executa toda a integração.

---------------------------------------------------------
9️⃣ REDIRECIONAMENTO COM window.open
---------------------------------------------------------

🔹 window.open(url, "_blank");

Função:
- Abre o WhatsApp em nova aba
- Mantém o site aberto
- Não recarrega a página atual

"_blank" → nova aba / nova janela

Isso melhora a experiência do usuário.

---------------------------------------------------------
🔟 COMO REUTILIZAR NO FUTURO
---------------------------------------------------------

A lógica NUNCA muda.
Somente o texto da mensagem muda.

Exemplos de reutilização:
- Orçamento
- Suporte
- Recrutador
- Vendas
- Atendimento

Formulário → mensagem → WhatsApp

---------------------------------------------------------
🔚 APRENDIZADO FINAL
---------------------------------------------------------

📌 Click to Chat é a forma mais simples de integrar WhatsApp  
📌 encodeURIComponent é obrigatório ao trabalhar com URLs  
📌 Soluções simples bem feitas são mais profissionais  
📌 Esse padrão é reutilizável em qualquer projeto front-end  

=========================================================
-->













<!--
🟢 BOLINHA PULSANTE (STATUS)

OBJETIVO:
Criar uma bolinha com efeito pulsante usando apenas
HTML + Tailwind CSS, sem JavaScript.

COMO É FEITO:
O efeito usa DOIS círculos no MESMO LUGAR:
• um fixo (visível sempre)
• um animado (pulso)

PASSO A PASSO:

1️⃣ Container com position: relative
<div class="relative flex items-center justify-center">

→ O relative é ESSENCIAL.
Ele define o ponto de referência para o círculo animado.
Sem isso, o absolute se posicionaria em outro lugar da tela.

2️⃣ Círculo animado (pulso)
<span class="absolute inline-flex h-3 w-3
  rounded-full bg-sky-400 opacity-75 animate-ping"></span>

→ absolute faz o círculo ficar exatamente em cima do outro  
→ animate-ping cria a animação de expansão  
→ opacity deixa o pulso mais suave e visualmente elegante  

3️⃣ Círculo fixo (base)
<span class="relative inline-flex h-3 w-3
  rounded-full bg-sky-400"></span>

→ Esse círculo garante que sempre exista uma bolinha visível,
mesmo quando o pulso estiver desaparecendo.

RESULTADO:
O círculo de trás cresce e some,
o da frente permanece fixo → efeito pulsante.

AJUSTES RÁPIDOS:
• Cor: bg-green-400 | bg-red-500
• Tamanho: h-2 w-2 | h-4 w-4
• Intensidade do pulso: opacity-50 / opacity-100
• Sem animação: remover animate-ping
-->













<!--
data: 24/01/2026
-->

<!--
====================================================================
APRENDIZADO: COMO FAZER VALIDAÇÃO DE FORMULÁRIO EM REACT (NA PRÁTICA)
====================================================================

Este aprendizado mostra COMO EU IMPLEMENTO uma validação de formulário
usando React, com código real, simples de entender e fácil de reutilizar
em qualquer outro projeto.

--------------------------------------------------------------------
1️⃣ CRIANDO O ESTADO DOS CAMPOS
--------------------------------------------------------------------

Primeiro, criamos estados para armazenar os valores digitados
pelo usuário em cada campo do formulário.

Exemplo real:

const [nome, setNome] = useState("");
const [email, setEmail] = useState("");
const [mensagem, setMensagem] = useState("");

Esses estados guardam os valores enquanto o usuário digita.

--------------------------------------------------------------------
2️⃣ CRIANDO O ESTADO DOS ERROS
--------------------------------------------------------------------

Depois, criamos estados SEPARADOS para armazenar mensagens de erro.
Cada campo tem seu próprio erro.

Exemplo real:

const [erroNome, setErroNome] = useState("");
const [erroEmail, setErroEmail] = useState("");
const [erroMensagem, setErroMensagem] = useState("");

Se a string estiver vazia (""), significa que o campo está válido.

--------------------------------------------------------------------
3️⃣ FUNÇÃO REAL DE VALIDAÇÃO DO NOME
--------------------------------------------------------------------

Aqui está uma função REAL usada no projeto:

const validarNome = (nome) => {
  if (!nome.trim()) return "Digite seu nome.";
  if (nome.trim().length < 3)
    return "O nome deve ter pelo menos 3 caracteres.";
  return "";
};

O que acontece aqui:
- trim() remove espaços vazios
- verifica se o campo está vazio
- verifica se tem pelo menos 3 caracteres
- retorna erro ou string vazia

--------------------------------------------------------------------
4️⃣ FUNÇÃO REAL DE VALIDAÇÃO DO EMAIL
--------------------------------------------------------------------

Validação de email feita com Regex (padrão de mercado):

const validarEmail = (email) => {
  const emailRegex = /^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/;

  if (!email.trim()) return "Digite seu email.";
  if (!emailRegex.test(email)) return "Digite um email válido.";
  return "";
};

Aqui garantimos que o email tenha o formato:
usuario@dominio.com

--------------------------------------------------------------------
5️⃣ FUNÇÃO REAL DE VALIDAÇÃO DA MENSAGEM
--------------------------------------------------------------------

const validarMensagem = (mensagem) => {
  if (!mensagem.trim()) return "Digite sua mensagem.";
  if (mensagem.trim().length < 10)
    return "A mensagem deve ter pelo menos 10 caracteres.";
  return "";
};

Isso evita mensagens vazias ou muito curtas.

--------------------------------------------------------------------
6️⃣ VALIDAÇÃO EM TEMPO REAL (onChange)
--------------------------------------------------------------------

Aqui está um input REAL com validação enquanto o usuário digita:

<input
  type="text"
  value={nome}
  onChange={(e) => {
    setNome(e.target.value);
    setErroNome(validarNome(e.target.value));
  }}
/>

O que acontece:
1. O valor do input é atualizado
2. A função de validação roda
3. O erro aparece ou some automaticamente

Isso melhora MUITO a experiência do usuário.

--------------------------------------------------------------------
7️⃣ MOSTRANDO O ERRO NA TELA
--------------------------------------------------------------------

Exemplo REAL de como mostrar o erro:

{erroNome && (
  <p className="text-red-400 text-sm mt-1">
    {erroNome}
  </p>
)}

Isso só aparece se existir erro.

--------------------------------------------------------------------
8️⃣ VALIDAÇÃO FINAL NO SUBMIT
--------------------------------------------------------------------

Mesmo com validação visual, SEMPRE validamos tudo no submit:

const enviarFormulario = (e) => {
  e.preventDefault();

  const erroNomeValidado = validarNome(nome);
  const erroEmailValidado = validarEmail(email);
  const erroMensagemValidada = validarMensagem(mensagem);

  setErroNome(erroNomeValidado);
  setErroEmail(erroEmailValidado);
  setErroMensagem(erroMensagemValidada);

  if (erroNomeValidado || erroEmailValidado || erroMensagemValidada) {
    return; // bloqueia o envio
  }

  // aqui o formulário pode ser enviado com segurança
};

Isso garante que NENHUM dado inválido passe.

--------------------------------------------------------------------
9️⃣ DESABILITANDO O BOTÃO DE ENVIO
--------------------------------------------------------------------

Exemplo REAL de bloqueio do botão:

const formularioInvalido =
  erroNome || erroEmail || erroMensagem || !nome || !email || !mensagem;

<button
  type="submit"
  disabled={formularioInvalido}
>
  Enviar mensagem
</button>

Se algo estiver errado, o botão fica desabilitado.

--------------------------------------------------------------------
🔟 COMO REPLICAR EM OUTRO PROJETO
--------------------------------------------------------------------

Para reutilizar esse padrão em outro projeto:

1. Crie estados para os campos
2. Crie estados para os erros
3. Crie funções de validação separadas
4. Valide no onChange (tempo real)
5. Valide novamente no submit
6. Bloqueie o envio se houver erro

Esse padrão funciona para:
- Formulário de contato
- Login
- Cadastro
- Qualquer formulário manual

====================================================================
FIM DO APRENDIZADO
====================================================================
-->











<!--
data: 26/01/2026

-->



<!--
==================================================
COMPONENTE: SOBRE
==================================================

OBJETIVO
--------
Criar uma seção "Sobre mim" moderna, organizada e responsiva,
usando React + Tailwind, com código limpo e fácil de manter.

--------------------------------------------------
1. ESTRUTURA BASE DA SECTION
--------------------------------------------------

<section
  id="sobre"
  className="
    w-full min-h-screen px-6 md:px-20 py-24
    bg-gradient-to-b from-[#020a1a] via-[#020617] to-[#020617]
    text-white
  "
>

- w-full / min-h-screen:
  Faz a seção ocupar toda a largura e pelo menos a altura da tela.

- px / py:
  Espaçamento interno lateral e vertical.

- bg-gradient-to-b:
  Gradiente escuro para diferenciar essa seção da anterior.

- text-white:
  Define o texto padrão como branco.

--------------------------------------------------
2. TÍTULO DA SEÇÃO
--------------------------------------------------

<div className="text-center max-w-3xl mx-auto mb-20">
  <h2 className="text-4xl md:text-5xl font-bold mb-4">
    Sobre mim
  </h2>

  <p className="text-gray-400 text-lg">
    Desenvolvedor Front-end com paixão por criar experiências digitais excepcionais
  </p>
</div>

- text-center:
  Centraliza o texto.

- max-w-3xl + mx-auto:
  Limita a largura e centraliza o bloco.

- Esse padrão é usado para criar uma introdução clara da seção.

--------------------------------------------------
3. GRID PRINCIPAL (TEXTO + CARDS)
--------------------------------------------------

<div className="grid grid-cols-1 lg:grid-cols-2 gap-16 max-w-7xl mx-auto">

- grid-cols-1:
  No mobile, tudo fica em uma coluna.

- lg:grid-cols-2:
  No desktop, divide em duas colunas.

- gap-16:
  Espaçamento entre as colunas.

--------------------------------------------------
4. BLOCO DE TEXTO (COLUNA ESQUERDA)
--------------------------------------------------

<div className="flex flex-col gap-6 text-gray-300 leading-relaxed">

Aqui usamos FLEX + COLUMN para empilhar os textos.

<p>
  Sou <span className="text-white font-semibold">Mikael Julião da Rocha</span>,
  desenvolvedor Front-end...
</p>

- span com text-white:
  Destaque em partes importantes do texto.

- leading-relaxed:
  Melhora a leitura aumentando o espaçamento entre linhas.

--------------------------------------------------
5. TECNOLOGIAS — USANDO ARRAY + MAP
--------------------------------------------------

<div className="flex flex-wrap gap-3">
  {[
    "React",
    "TypeScript",
    "JavaScript (ES6+)",
    "Tailwind CSS",
    "HTML5",
    "CSS3",
    "Git & GitHub",
    "Figma"
  ].map((tech) => (
    <span
      key={tech}
      className="
        px-4 py-2 rounded-full text-sm
        bg-slate-900/60 border border-white/10
        text-gray-200
      "
    >
      {tech}
    </span>
  ))}
</div>

EXPLICAÇÃO:
- Criamos um array de tecnologias.
- O map percorre o array.
- Para cada item, renderizamos um <span>.

Vantagens:
- Código menor
- Fácil adicionar/remover tecnologias
- Evita repetição








--------------------------------------------------
6. CARDS INFORMATIVOS — ARRAY DE OBJETOS + MAP
--------------------------------------------------

AQUI ESTÁ O PULO DO GATO DO COMPONENTE.

Em vez de criar vários cards "na mão", repetindo
div, h3, p, ícone, etc., criamos uma LISTA DE DADOS
e deixamos o React gerar a interface pra gente.

EXEMPLO DO ARRAY:

const cards = [
  {
    icon: "bx bx-code-alt",
    title: "Experiência",
    text: "12 meses de experiência em desenvolvimento front-end..."
  },
  {
    icon: "bx bx-palette",
    title: "Background em Design",
    text: "Mais de 1,5 ano de experiência em Design Gráfico..."
  }
];

CADA OBJETO REPRESENTA UM CARD.
- icon → classe do ícone (Boxicons)
- title → título do card
- text → texto descritivo

Ou seja:
📦 DADOS PRIMEIRO
🎨 INTERFACE DEPOIS

--------------------------------------------------
COMO O MAP FUNCIONA NA PRÁTICA
--------------------------------------------------

{cards.map((card, index) => (
  <div key={index} className="bg-slate-900/40 border border-white/10 rounded-xl p-6">

    <i className={`${card.icon} text-2xl text-sky-400 mb-4`}></i>

    <h3 className="text-lg font-semibold mb-2">
      {card.title}
    </h3>

    <p className="text-gray-400 text-sm">
      {card.text}
    </p>

  </div>
))}

O QUE ACONTECE AQUI:

1️⃣ O map percorre o array "cards"
2️⃣ Para cada objeto:
   - card.icon vira o ícone
   - card.title vira o título
   - card.text vira o texto
3️⃣ O React cria UM CARD PARA CADA OBJETO

Se o array tiver:
- 4 objetos → 4 cards
- 10 objetos → 10 cards

SEM mudar NADA no HTML.

--------------------------------------------------
POR QUE ISSO É IMPORTANTE?
--------------------------------------------------

✔ Código mais curto
✔ Código mais fácil de entender
✔ Muito mais fácil de manter
✔ Escala sem dor de cabeça

Exemplo:
Para adicionar um novo card, você NÃO cria HTML.
Você só adiciona um novo objeto no array.

--------------------------------------------------
7. MÉTRICAS — DADOS + IMPACTO VISUAL
--------------------------------------------------

A lógica aqui é A MESMA dos cards,
só que aplicada a números de impacto.

ARRAY DE MÉTRICAS:

const metrics = [
  { value: "12+", label: "Meses de experiência" },
  { value: "5+", label: "Projetos entregues" },
  { value: "1.5+", label: "Anos em Design" },
  { value: "100%", label: "Comprometimento" }
];

Cada objeto tem:
- value → número grande (impacto visual)
- label → explicação do número

--------------------------------------------------
RENDERIZAÇÃO COM MAP
--------------------------------------------------

{metrics.map((item, index) => (
  <div key={index}>
    <p className="text-4xl font-bold text-sky-400">
      {item.value}
    </p>

    <p className="text-gray-400">
      {item.label}
    </p>
  </div>
))}

Aqui:
- value chama atenção (olho do usuário)
- label explica o significado
- map mantém tudo organizado e consistente

--------------------------------------------------
COMO PENSAR ESSE PADRÃO NO FUTURO
--------------------------------------------------

Sempre que você pensar:
"vou repetir esse bloco várias vezes"

PENSE:
👉 Isso pode virar um array + map?

Se a resposta for sim,
você está no caminho certo 🚀

--------------------------------------------------
APRENDIZADOS DESSE COMPONENTE
--------------------------------------------------

- Separar DADOS da INTERFACE
- Usar map de forma real (não só teoria)
- Evitar repetição de código
- Criar componentes escaláveis
- Pensar como desenvolvedor front-end profissional

Esse padrão é MUITO usado em projetos reais.
Aprender isso aqui é um divisor de águas.
-->























<!--
data: 27/01/2026







════════════════════════════════════════════════════════════════════
📘 PARTE 1 — FUNDAMENTOS DO COMPONENTE (DO ZERO)
════════════════════════════════════════════════════════════════════

Este comentário explica TODO o raciocínio do componente "Experiencia",
pensando em alguém que ainda está aprendendo React, JSX e Tailwind.

────────────────────────────────────────────────────────────
🧠 1. O QUE ESSE COMPONENTE FAZ?
────────────────────────────────────────────────────────────

Este componente cria uma TIMELINE PROFISSIONAL, ou seja:
- uma linha vertical no meio da tela
- bolinhas marcando momentos importantes
- cards com informações de cada experiência

Visualmente:
- no DESKTOP: os cards alternam esquerda e direita
- no MOBILE: os cards ficam centralizados, um abaixo do outro

────────────────────────────────────────────────────────────
📦 2. POR QUE USAMOS UM ARRAY DE OBJETOS?
────────────────────────────────────────────────────────────

Em vez de escrever vários cards "na mão",
criamos um ARRAY que guarda os dados.

Isso permite:
- menos código repetido
- mais organização
- facilidade para adicionar novas experiências

Código usado no projeto:

const experiencias = [
  {
    empresa: "Freelancer Autônomo",
    cargo: "Desenvolvedor Front-end",
    periodo: "Nov/2024 – Atual",
    descricao: [
      "Desenvolvimento de interfaces web responsivas utilizando React, JavaScript (ES6+), HTML e CSS",
      "Criação de componentes reutilizáveis seguindo princípios de Clean Code",
      "Estilização de aplicações com Tailwind CSS e Bootstrap conforme a necessidade do projeto",
      "Implementação de melhorias de UX/UI, garantindo navegação intuitiva e acessibilidade básica",
      "Desenvolvimento de landing pages focadas em performance, conversão e responsividade",
      "Manutenção, refatoração de código e correção de bugs em projetos existentes"
    ]
  },
  {
    empresa: "CustomFashion",
    cargo: "Designer Gráfico Digital",
    periodo: "Jan/2024 – Jun/2025",
    descricao: [
      "Criação de identidades visuais, logotipos e materiais gráficos digitais e impressos",
      "Tratamento e edição de imagens para impressão em alta qualidade",
      "Desenvolvimento de artes para banners, estampas e materiais promocionais",
      "Atuação com foco em estética, consistência visual e experiência do usuário",
      "Utilização profissional de CorelDRAW e Adobe Photoshop"
    ]
  }
];

Cada objeto representa UMA experiência.
Cada campo vira um pedaço do card.

────────────────────────────────────────────────────────────
🔁 3. COMO O React TRANSFORMA DADOS EM INTERFACE?
────────────────────────────────────────────────────────────

Usamos o método map() para percorrer o array
e gerar um card para cada item:

{experiencias.map((item, index) => { ... })}

O map():
- percorre cada objeto do array
- executa o JSX para cada experiência
- evita escrever código repetido

────────────────────────────────────────────────────────────
📐 4. SECTION PRINCIPAL (BASE DO LAYOUT)
────────────────────────────────────────────────────────────

<section
  id="experiencias"
  className="
    relative
    w-full
    py-32
    text-white
    bg-gradient-to-b
    from-[#020617]
    to-[#021024]
  "
>

Explicando classe por classe:
- relative → permite usar position absolute nos filhos
- w-full → ocupa toda a largura da tela
- py-32 → espaço grande em cima e embaixo
- bg-gradient → fundo elegante e profissional

Essa section é o "palco" onde tudo acontece.

────────────────────────────────────────────────────────────
🧵 5. LINHA CENTRAL DA TIMELINE
────────────────────────────────────────────────────────────

<div
  className="
    absolute
    left-1/2
    top-48
    bottom-0
    w-[2px]
    bg-white/10
    -translate-x-1/2
    z-10
  "
/>

O que isso faz:
- absolute → não empurra nada do layout
- left-1/2 → joga para o meio da tela
- translate-x-1/2 → corrige o alinhamento
- w-[2px] → linha fina
- z-10 → fica atrás dos cards

Essa linha é fixa e serve como referência visual.

════════════════════════════════════════════════════════════════════















════════════════════════════════════════════════════════════════════
📘 PARTE 2 — RESPONSIVIDADE, BOLINHA E CAMADAS (Z-INDEX)
════════════════════════════════════════════════════════════════════

Aqui entram os detalhes mais avançados do componente,
responsáveis pelo acabamento profissional.

────────────────────────────────────────────────────────────
↔️ 6. ESQUERDA OU DIREITA NO DESKTOP
────────────────────────────────────────────────────────────

Dentro do map(), usamos o índice para alternar os cards:

const esquerda = index % 2 === 0;

Isso significa:
- índice par → card à esquerda
- índice ímpar → card à direita

No JSX:

<div
  className={`relative flex w-full justify-center
    ${esquerda ? "md:justify-start" : "md:justify-end"}
  `}
>

No mobile:
- sempre centralizado (justify-center)

No desktop:
- md:justify-start / md:justify-end alternam os lados

────────────────────────────────────────────────────────────
📱 7. O PROBLEMA DO MOBILE (E A SOLUÇÃO)
────────────────────────────────────────────────────────────

No mobile, a linha ficava aparecendo atrás do card,
o que deixava o visual estranho.

Para resolver isso, criamos um "recorte invisível":

<div
  className="
    block md:hidden
    absolute
    left-1/2
    -translate-x-1/2
    w-10
    h-full
    bg-[#020617]
    z-20
  "
/>

Esse elemento:
- só existe no mobile
- tem a mesma cor do fundo
- fica acima da linha
- ESCONDE a linha atrás do card

Isso é um truque visual muito usado em layouts profissionais.

────────────────────────────────────────────────────────────
🔵 8. BOLINHA DA TIMELINE
────────────────────────────────────────────────────────────

<span
  className="
    absolute
    left-1/2
    -top-8 md:top-8
    w-4 h-4
    rounded-full
    bg-sky-400
    -translate-x-1/2
    z-30
  "
/>

Detalhes importantes:
- sempre alinhada ao centro da linha
- no mobile:
  -top-8 → cria espaço entre bolinha e card
- no desktop:
  md:top-8 → alinha com a linha

z-30 garante que ela fique acima de tudo.

────────────────────────────────────────────────────────────
🪟 9. CARD SEMPRE NA FRENTE
────────────────────────────────────────────────────────────

<div
  className="
    relative
    z-30
    w-full md:max-w-md
    bg-slate-900
    border border-white/10
    rounded-xl
    p-6
    md:translate-x-16 ou md:-translate-x-16
  "
>

Aqui acontece a mágica final:
- z-30 → card sempre acima da linha
- background sólido → cobre o que estiver atrás
- translate-x → afasta o card da linha no desktop

────────────────────────────────────────────────────────────
🎯 CONCLUSÃO FINAL
────────────────────────────────────────────────────────────

Esse componente ensina:
- uso real de map() em React
- controle de layout com Tailwind
- posicionamento com absolute / relative
- uso correto de z-index
- responsividade profissional

Esse padrão pode ser reutilizado em QUALQUER projeto
que precise de uma timeline moderna e elegante.

════════════════════════════════════════════════════════════════════
-->







